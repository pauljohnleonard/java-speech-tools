...
import com.frinika.audio.io.AudioWavReader;
import com.frinika.audio.io.BlockableAudioProcess;
 ...
 */
public class MyAudioReader extends AudioWavReader implements AudioProcess{
	...
	public MyAudioReader(RandomAccessFileIF fisIF, float Fs) throws IOException {
		super(fisIF.getRandomAccessFile());

		sampleRate = format.getSampleRate();
		if (sampleRate != Fs) {
			try {
				throw new Exception(" audio file is not correct sample rate."
						+ sampleRate + " should be" + Fs);
			} catch (Exception ex) {
				Logger.getLogger(MyAudioReader.class.getName()).log(
	...
	// /**
	// *
	// * this version will block if the file is being written to and there is
	// not enough
	...
	public int processAudio(AudioBuffer buffer) {
		...
		int nread;
		try {
			nread = bfis.read(byteBuff, 0, nBytes, realTime);
		
		if (nread != nBytes)
			try {
				throw new Exception(" Ooops only read " + nread + " out of "
						+ nBytes);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
